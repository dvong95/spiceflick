...
import { Navbar, Button, NavbarItem, Dropdown, DropdownItem, DropdownTrigger, DropdownMenu, User, Input, DropdownSection } from "@nextui-org/react";
import { FiMessageCircle, FiMoreHorizontal } from "react-icons/fi";
import IUser from '../models/IUser';
import users from '../data/Users';
import ThemeSwitcher from './ThemeSwitch';

const NavigationBar: React.FC<{ user: IUser }> = ({ user }) => {
      ...
      <Navbar className="flex py-2 text-foreground w-full">
        ...
        <div className="flex space-x-4">
          ...
          <NavbarItem>
            <Dropdown>
              <DropdownTrigger>
                <Button variant="flat" className="list-none">Demo</Button>
              </DropdownTrigger>
              <DropdownMenu>
          ...
          <NavbarItem>
            <Dropdown closeOnSelect={false}>
              <DropdownTrigger>
                <Button variant="flat" className="list-none">Pages</Button>
              </DropdownTrigger>
              <DropdownMenu aria-label="Static Actions">
                <DropdownItem>
                  <Dropdown>
                    <DropdownTrigger>
                      <Button variant="flat" color='primary'>Albums</Button>
                    </DropdownTrigger>
                    <DropdownMenu>
          ...
          <NavbarItem>
            <Dropdown>
              <DropdownTrigger>
                <Button variant="flat" className="bg-blue-500 text-white hover:bg-blue-600 p-2">
                  <FiMessageCircle size={30} />
                </Button>
              </DropdownTrigger>
              <DropdownMenu className="max-h-48 overflow-y-scroll w-80">
                <DropdownSection className='sticky top-0 text-default-foreground'>
                  <DropdownItem key="open-messages">
                    <Button variant="flat" className="w-full mb-2">Open Messages Tab</Button>
                  </DropdownItem>
                </DropdownSection>
                {users.map((dm, index) => (
                  <DropdownSection className='max-h-[100px] overflow-hidden'>
                  <DropdownItem key={`dm${dm.id}`} className="flex items-center justify-between space-x-2 ">
                    <div className='flex justify-between'>
                      <div className="flex items-center space-x-2">
                        <User avatarProps={{ src: dm.profilePicture }} />
                        <div>
                          <p className="text-sm font-bold">{dm.name}</p>
                          <p className="text-sm max-w-[150px] text-gray-500 truncate text-ellipsis">{dm.message}</p>
                        </div>
                      </div>
                      <div>
            ...
            <Dropdown>
              ...
              <DropdownMenu aria-label="User Actions">
                <DropdownSection aria-label="Profile & Actions" showDivider>
                  <DropdownItem key="profile" className="flex items-start flex-col gap-1">
